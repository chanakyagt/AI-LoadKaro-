...
export default function AdminVerificationsPage() {
  ...
  const fetchPending = useCallback(async () => {
    ...
    setError(null);
    try {
      const [usersRes, trucksRes, subsRes] = await Promise.all([
        fetch(
        ...
        fetch(
          `/api/admin/verification/submissions?entity_type=user&status=submitted`,
          { credentials: "include" }
        ),
      ]);
      ...
      const subs = subsRes.ok ? ((await subsRes.json()) as { data: Record<string, unknown>[] }).data : [];

      const truckSubsRes = await fetch(
        `/api/admin/verification/submissions?entity_type=truck&status=submitted`,
        { credentials: "include" }
      );
      const truckSubs = truckSubsRes.ok ? ((await truckSubsRes.json()) as { data: Record<string, unknown>[] }).data : [];

      const subsByEntity = new Map<string, string>();
      [...subs, ...truckSubs].forEach((s) => {
        const key = `${s.entity_type}-${s.entity_id}`;
        if (!subsByEntity.has(key)) {
          subsByEntity.set(key, String(s.created_at ?? ""));
        }
      });
      ...
      users.forEach((u) => {
        const subDate = subsByEntity.get(`user-${u.id}`) ?? String(u.created_at ?? "");
        result.push({
          id: String(u.id),
          entityType: "user",
          name: String(u.name ?? "—"),
          ...
          submittedAt: subDate,
          docsCount: 0,
          daysWaiting: daysFromNow(subDate),
        });
      });

      trucks.forEach((t) => {
        const owner = t.users as { name?: string } | null;
        const subDate = subsByEntity.get(`truck-${t.id}`) ?? String(t.created_at ?? "");
        result.push({
          id: String(t.id),
          entityType: "truck",
          name: owner?.name ?? "—",
    ...
    <div className="space-y-5">
      <div>
        ...
        <p className="text-sm text-[#6b7280]">
          All pending verifications across users and trucks, sorted by wait time (oldest first).
        </p>
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <Card className="border-[#e5e7eb] bg-white shadow-sm">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-[#6b7280]">Total pending</CardTitle>
          </CardHeader>
          <CardContent>
            <p className="text-3xl font-bold tabular-nums text-[#111827]">{items.length}</p>
          </CardContent>
        </Card>
        <Card className="border-[#e5e7eb] bg-white shadow-sm">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-[#6b7280]">Users</CardTitle>
          </CardHeader>
          <CardContent>
            <p className="text-3xl font-bold tabular-nums text-amber-600">{userCount}</p>
          </CardContent>
        </Card>
        <Card className="border-[#e5e7eb] bg-white shadow-sm">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-[#6b7280]">Trucks</CardTitle>
          </CardHeader>
          <CardContent>
            <p className="text-3xl font-bold tabular-nums text-blue-600">{truckCount}</p>
          </CardContent>
        </Card>
      </div>
        ...
        <Table>
          ...
          <TableBody>
            {loading ? (
              ...
              <TableRow>
                <TableCell colSpan={6} className="py-12 text-center text-[#9ca3af]">
              ...
              filtered.map((item) => (
                <TableRow key={`${item.entityType}-${item.id}`} className="border-[#e5e7eb]">
                  <TableCell>
                    ...
                    >
                      {item.entityType === "user" ? "User" : "Truck"}
                    </Badge>
                  </TableCell>
      ...
      {docsTarget && (
        <VerificationDocsModal
          ...
          entityType={docsTarget.entityType}
